...
# Clear the file directory containing the segmented images
def clear_dir(dst_dir):
    for file_name in os.listdir(dst_dir):
        os.remove(os.path.join(dst_dir, file_name))


# Update the dictionary with the new results generated from the segmented image
...
# Normalize the segmented image before passing it to the model
...
# Display the content on a full screen
...
# Add a border to the square image so that it fits the full screen without clipping edges of the square image
...
# Draw the bounding box and labels on the initial unsegmented image
...
# The code that runs during the demonstration
...
# The main loop that is run during the demonstration
def main():

    dst_dir = rf'/home/lewocp/Desktop/lego-classification-using-ml/segmented-testing-images'
    src_dir = rf'/home/lewocp/Desktop/lego-classification-using-ml/testing-images'
    model = tf.keras.models.load_model('/home/lewocp/Desktop/lego-classification-using-ml/neural_net')
    ...
    if cap.isOpened():
        ...
        if _ and frame is not None:
            cv.imwrite(rf'/home/lewocp/Desktop/lego-classification-using-ml/testing-images/0.png', frame)
