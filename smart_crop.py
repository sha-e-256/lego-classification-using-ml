...
def get_threshold_and_contours(img):
    img_no_bg, img_bg = clear_background(img)
    img_grey = cv.cvtColor(img_no_bg, cv.COLOR_BGR2GRAY)
...
def clear_background(img):
    dark_colour = np.array([255, 255, 255])  # HSV for all
    light_colour = np.array([0, 100, 0])  # washed out colours
    img_hsv = cv.cvtColor(img, cv.COLOR_BGR2HSV)
    img_mask = cv.inRange(src=img_hsv,
                          lowerb=light_colour,
                          upperb=dark_colour)
    img_mask_white = cv.bitwise_not(img_mask)
    img[img_mask_white == 255] = WHITE
    return img, img_mask_white
...
def smart_crop(img, dst_dir, isTest):
    ...
    while i < len(contours_descending_size):
        ...
        if area < 34200:  # 380 x 90 (4x1 plate: smallest piece)
