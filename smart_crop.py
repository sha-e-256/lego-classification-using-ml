...
def get_threshold_and_contours(img):
    img_copy = img.copy()
    dark_colour = np.array([255, 255, 255])  # HSV for dark grey
    light_colour = np.array([0, 150, 0])  # Light grey
    img_hsv = cv.cvtColor(img, cv.COLOR_BGR2HSV)
    img_mask = cv.inRange(src=img_hsv,
                          lowerb=light_colour,
                          upperb=dark_colour)
    cv.imshow('win', img_mask)
    cv.waitKey()
    cv.destroyAllWindows()

    img_copy[img_mask < 210] = WHITE

    img_grey = cv.cvtColor(img_copy, cv.COLOR_BGR2GRAY)
